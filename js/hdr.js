function getImageArray(display, img) {
	...
	ctx.drawImage(img, 0, 0, Math.min(img.width, display.width), Math.min(img.height, display.height));
...
function toneMapFattal(display, image, alpha, beta) {
	...
	while (true) {
		...
		for (var x = 0; x < width; x ++) {
			...
			(source[jSource]
			+ source[jSource + 1]
			+ source[jSource + width * 2]
			+ source[jSource + 1 + width * 2]) * 0.25;
		    jSource += 2;
		}
	    }
	    mipmap.push(target);
	...
	for (var l = 0; l < maxiter; l ++) {
		...
		for (var y = 0; y < height; y ++) {
			...
			/* left corner pixel */
			if (xinit == 0) {
			    var defect = approximation[idx + 1]
				+ approximation[idx + rowOffset]
				- 4 * approximation[idx]
				- grid[idx];
...
var hdr = function(result) {
	//set.onchange = function() {
	buildImageSet(result);
	// };
	//set.onchange();
}

function buildImageSet(set) {
	...
	var images = [];
	img.onload = function() {
		var me = getImageArray(display, this);
		images.push(me);
		console.log(set.length);
		console.log(i);
		/* when all images have been loaded, process HDR & rest */
		if (i == set.length) {
			var hdrImage = buildHdrImage(display, images);
			buildControls(display, images, hdrImage);
			console.log("!!!hahah");
		} else {
			this.src = set[i++];
		}
...
function buildControls(display, images, hdrImage) {
    var ctx = display.getContext('2d');
    var select = document.getElementById('alternatives');
	...
	select[i] = new Option("source " + i);
    }
    select[i] = new Option("raw HDR");
    select[i+1] = new Option("global map");
    select[i+2] = new Option("local map");
    select.selectedIndex = i+1;
   
    a.onchange = alpha.onchange = beta.onchange = bottom.onchange = top.onchange = select.onchange = function() {
	var chosenImage = null;
	var bottomValue = parseFloat(bottom.value);
	var topValue = parseFloat(top.value);
	var aValue = parseFloat(a.value);
	var alphaValue = parseFloat(alpha.value);
	var betaValue = parseFloat(beta.value);
	
	if (select.selectedIndex < images.length) {
	    chosenImage = images[select.selectedIndex];
	} else if (select.selectedIndex == images.length) {
	    chosenImage = hdrImage;
	} else if (select.selectedIndex == images.length + 1) {
	    chosenImage = toneMapGlobal(display, hdrImage, aValue);
	} else if (select.selectedIndex == images.length + 2) {
		console.log("aaa");
		console.log(alphaValue);
		console.log(betaValue);

	    chosenImage = toneMapFattal(display, hdrImage, alphaValue, betaValue);
	}

	//log("Displaying selection: " + select[select.selectedIndex].text);
	ctx.putImageData(exposureApplier(display, bottomValue, topValue, chosenImage), 0, 0);
    }
    select.onchange();
}
